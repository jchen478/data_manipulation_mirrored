...
int main(){
	...
	for (step = 0; step < nConfig; step++){
		...
		fwrite(&dum, sizeof(float), 1, rxfileTmp); 
		fwrite(r, sizeof(float), nfib*nseg, rxfileTmp);
		// ry
		fread(&dum, sizeof(float), 1, ryfile);
		fread(r, sizeof(float), nfib*nseg, ryfile);
		fwrite(&dum, sizeof(float), 1, ryfileTmp);
		fwrite(r, sizeof(float), nfib*nseg, ryfileTmp);
		// rz
		fread(&dum, sizeof(float), 1, rzfile);
		fread(r, sizeof(float), nfib*nseg, rzfile);
		fwrite(&dum, sizeof(float), 1, rzfileTmp);
		fwrite(r, sizeof(float), nfib*nseg, rzfileTmp);
		// px 
		fread(&dum, sizeof(float), 1, pxfile);
		fread(r, sizeof(float), nfib*nseg, pxfile);
		fwrite(&dum, sizeof(float), 1, pxfileTmp);
		fwrite(r, sizeof(float), nfib*nseg, pxfileTmp);
		// py
		fread(&dum, sizeof(float), 1, pyfile);
		fread(r, sizeof(float), nfib*nseg, pyfile);
		fwrite(&dum, sizeof(float), 1, pyfileTmp);
		fwrite(r, sizeof(float), nfib*nseg, pyfileTmp);
		// pz
		fread(&dum, sizeof(float), 1, pzfile);
		fread(r, sizeof(float), nfib*nseg, pzfile);
		fwrite(&dum, sizeof(float), 1, pzfileTmp);
		fwrite(r, sizeof(float), nfib*nseg, pzfileTmp);
		// ux 
		fread(&dum, sizeof(float), 1, uxfile);
		fread(r, sizeof(float), nfib*nseg, uxfile);
		fwrite(&dum, sizeof(float), 1, uxfileTmp);
		fwrite(r, sizeof(float), nfib*nseg, uxfileTmp);
		// uy 
		fread(&dum, sizeof(float), 1, uyfile);
		fread(r, sizeof(float), nfib*nseg, uyfile);
		fwrite(&dum, sizeof(float), 1, uyfileTmp);
		fwrite(r, sizeof(float), nfib*nseg, uyfileTmp);
		// uy 
		fread(&dum, sizeof(float), 1, uzfile);
		fread(r, sizeof(float), nfib*nseg, uzfile);
		fwrite(&dum, sizeof(float), 1, uzfileTmp);
		fwrite(r, sizeof(float), nfib*nseg, uzfileTmp);
		// wx 
		fread(&dum, sizeof(float), 1, wxfile);
		fread(r, sizeof(float), nfib*nseg, wxfile);
		fwrite(&dum, sizeof(float), 1, wxfileTmp);
		fwrite(r, sizeof(float), nfib*nseg, wxfileTmp);
		// wy
		fread(&dum, sizeof(float), 1, wyfile);
		fread(r, sizeof(float), nfib*nseg, wyfile);
		fwrite(&dum, sizeof(float), 1, wyfileTmp);
		fwrite(r, sizeof(float), nfib*nseg, wyfileTmp);
		// wz
		fread(&dum, sizeof(float), 1, wzfile);
		fread(r, sizeof(float), nfib*nseg, wzfile);
		fwrite(&dum, sizeof(float), 1, wzfileTmp);
		fwrite(r, sizeof(float), nfib*nseg, wzfileTmp);
		// q0
		fread(&dum, sizeof(float), 1, q0file);
		fread(r, sizeof(float), nfib*nseg, q0file);
		fwrite(&dum, sizeof(float), 1, q0fileTmp);
		fwrite(r, sizeof(float), nfib*nseg, q0fileTmp);
		// q1
		fread(&dum, sizeof(float), 1, q1file);
		fread(r, sizeof(float), nfib*nseg, q1file);
		fwrite(&dum, sizeof(float), 1, q1fileTmp);
		fwrite(r, sizeof(float), nfib*nseg, q1fileTmp);
		// q2
		fread(&dum, sizeof(float), 1, q2file);
		fread(r, sizeof(float), nfib*nseg, q2file);
		fwrite(&dum, sizeof(float), 1, q2fileTmp);
		fwrite(r, sizeof(float), nfib*nseg, q2fileTmp);
		// q3
		fread(&dum, sizeof(float), 1, q3file);
		fread(r, sizeof(float), nfib*nseg, q3file);
		fwrite(&dum, sizeof(float), 1, q3fileTmp);
		fwrite(r, sizeof(float), nfib*nseg, q3fileTmp);
	}
